...
import {
  AlertTriangle, Users, Droplets, Megaphone,
  BarChart3, FileText, Shield, LayoutDashboard, Globe, Home,
  Settings, ChevronDown, ClipboardCheck, Wrench, BookOpen
} from 'lucide-react';
...
export default function Sidebar() {
  ...
  const childPaths = useMemo(() => {
    ...
    navGroups.forEach((g) => g.items.forEach((item) => {
      if (item.children) map[item.label] = item.children.map((c) => c.to);
    }));
    return map;
  }, []);
  ...
  const activeParent = useMemo(() => {
    for (const [label, paths] of Object.entries(childPaths)) {
      if (paths.some((p) => pathname.startsWith(p))) return label;
    }
    return null;
  }, [pathname, childPaths]);
  ...
  const toggle = (label: string) =>
    setOpenTrees((prev) => ({ ...prev, [label]: !prev[label] }));

  const isParentActive = (label: string) => activeParent === label;

  // Auto-open parent when a child is active
  const open = (label: string) => openTrees[label] ?? isParentActive(label);
  ...
  return (
    <aside className="w-64 h-screen bg-white border-r border-gray-200 flex flex-col fixed left-0 top-0 z-30">
      <div className="h-14 flex items-center gap-2 px-4 border-b border-gray-200">
        <img src="/bfp-logo.png" alt="BFP" className="w-8 h-8 rounded-lg object-cover" />
        <div>
          <div className="text-sm font-semibold text-gray-900">BFP Ipil Station</div>
          <div className="text-[10px] text-gray-500">Management System</div>
        </div>
      </div>
            ...
            {group.items.map((item) =>
              item.children ? (
                <div key={item.label}>
                  <button
                    ...
                    className={`w-full flex items-center gap-2 px-2 py-1.5 text-sm rounded-md ${
                      isParentActive(item.label) ? 'bg-red-50 text-red-700 font-medium' : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                    <item.icon size={16} />
                    <span className="flex-1 text-left">{item.label}</span>
                    {item.badge && (
                      <span className="text-[10px] font-medium bg-red-100 text-red-700 px-1.5 py-0.5 rounded-full">{item.badge}</span>
                    )}
                    <ChevronDown size={14} className={`transition-transform ${open(item.label) ? 'rotate-0' : '-rotate-90'}`} />
                  </button>
                  {open(item.label) && (
                    <div className="ml-6 space-y-0.5">
                      {item.children.map((child) => (
                        <NavLink
                          ...
                          className={({ isActive }) =>
                            `block px-2 py-1 text-sm rounded-md ${isActive ? 'bg-red-50 text-red-700 font-medium' : 'text-gray-600 hover:bg-gray-100'}`
                          }
                        >
                          {child.label}
                        </NavLink>
                ...
                <NavLink
                  ...
                  to={item.to!}
                  className={({ isActive }) =>
                    `flex items-center gap-2 px-2 py-1.5 text-sm rounded-md ${isActive ? 'bg-red-50 text-red-700 font-medium' : 'text-gray-700 hover:bg-gray-100'}`
                  }
                >
                  <item.icon size={16} />
                  {item.label}
      ...
      <div className="border-t border-gray-200 p-3">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-full bg-gray-200 flex items-center justify-center text-xs font-medium text-gray-600">AD</div>
          <div>
            <div className="text-xs font-medium text-gray-900">Admin User</div>
            <div className="text-[10px] text-gray-500">Fire Officer</div>
          </div>
        </div>
      </div>
    </aside>
