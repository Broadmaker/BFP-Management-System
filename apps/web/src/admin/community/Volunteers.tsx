...
export default function Volunteers() {
  ...
  useEffect(() => {
    VolunteersApi.list().then((data) => { setVolunteers(data); setLoading(false); }).catch(() => setLoading(false));
  }, []);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">{activeCount} active volunteers</p>
        </div>
          ...
          <tbody>
            {filtered.map((v) => (
              <tr key={v.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-2.5 text-gray-600">{v.contactNumber}</td>
                ...
                <td className="px-4 py-2.5">
                  <span className={`text-[11px] font-medium px-1.5 py-0.5 rounded-full ${v.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
