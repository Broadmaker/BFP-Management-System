...
export default function OutgoingDocuments() {
    ...
    useEffect(() => {
        DocumentsApi.list().then((data) => {
            setItems(data.filter((d) => d.type === 'outgoing').map(parseMeta));
            setLoading(false);
        }).catch(() => setLoading(false));
    }, []);
    async function save() {
        if (!form.title || !form.recipient)
            return;
    ...
    const filtered = items.filter((r) => {
        ...
        if (search) {
            ...
            return r.title.toLowerCase().includes(q) || r.recipient.toLowerCase().includes(q) || r.tracking.toLowerCase().includes(q);
        }
        return true;
    });
