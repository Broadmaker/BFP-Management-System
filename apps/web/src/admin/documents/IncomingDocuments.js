...
export default function IncomingDocuments() {
    ...
    useEffect(() => {
        DocumentsApi.list().then((data) => {
            setItems(data.filter((d) => d.type === 'incoming').map(parseMeta));
            setLoading(false);
