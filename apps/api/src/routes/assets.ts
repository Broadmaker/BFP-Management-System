...
app.get('/equipment', async (c) => {
  ...
  const items = await db.select().from(s.equipment).all();
  return c.json(items);
});
...
app.get('/vehicles', async (c) => {
  const db = createDb(c.env.DB);
  const items = await db.select().from(s.vehicles).all();
  return c.json(items);
});
...
app.get('/maintenance', async (c) => {
  const db = createDb(c.env.DB);
  const items = await db.select().from(s.maintenanceRecords).all();
  return c.json(items);
});
...
app.get('/fuel', async (c) => {
  const db = createDb(c.env.DB);
  const items = await db.select().from(s.fuelLogs).all();
  return c.json(items);
});
